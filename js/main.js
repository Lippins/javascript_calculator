...
backspace.addEventListener("click", () => {
  displayScreen.innerHTML = displayScreen.innerHTML.slice(
    0,
    displayScreen.innerHTML.length - 1
  );
});
...
operators.forEach((operator) => {
  operator.addEventListener("click", () => {
    if (displayScreen.innerHTML.length >= 1) {
      //if there is no digit don't initialise an operator
      if (
        displayScreen.innerHTML.slice(-1) == "÷" || //eliminate consecutive operators
        displayScreen.innerHTML.slice(-1) == "-" ||
        displayScreen.innerHTML.slice(-1) == "×" ||
        displayScreen.innerHTML.slice(-1) == "+" ||
        displayScreen.innerHTML.slice(-1) == "^" ||
        ...
        displayScreen.innerHTML =
          displayScreen.innerHTML.substring(
            0,
            displayScreen.innerHTML.length - 1
          ) + operator.innerHTML;
        resultInDisplay = false;
...
result.addEventListener("click", () => {
  ...
  for (power; power != -1; power = expressionOperators.indexOf("^")) {
    expressionNumbers.splice(
      power,
      2,
      Math.pow(expressionNumbers[power], expressionNumbers[power + 1])
    );
  ...
  for (sqRoot; sqRoot != -1; sqRoot = expressionOperators.indexOf("√")) {
    expressionNumbers.splice(
      sqRoot + 1,
      1,
      Math.sqrt(expressionNumbers[sqRoot + 1])
    );
  ...
  for (divide; divide != -1; divide = expressionOperators.indexOf("÷")) {
    expressionNumbers.splice(
      divide,
      2,
      expressionNumbers[divide] / expressionNumbers[divide + 1]
    );
  ...
  for (times; times != -1; times = expressionOperators.indexOf("×")) {
    expressionNumbers.splice(
      times,
      2,
      expressionNumbers[times] * expressionNumbers[times + 1]
    );
  ...
  for (plus; plus != -1; plus = expressionOperators.indexOf("+")) {
    expressionNumbers.splice(
      plus,
      2,
      parseFloat(expressionNumbers[plus]) +
        parseFloat(expressionNumbers[plus + 1])
    );
  ...
  for (minus; minus != -1; minus = expressionOperators.indexOf("-")) {
    expressionNumbers.splice(
      minus,
      2,
      expressionNumbers[minus] - expressionNumbers[minus + 1]
    );
  ...
  if (
    displayScreen.innerHTML.length > 12 &&
    displayScreen.innerHTML.indexOf(".") > 0
  ) {
